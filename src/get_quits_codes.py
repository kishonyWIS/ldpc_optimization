...
def init_code(code, cx_order='lex'):
    ...
    if cx_order == 'lex':
        ...
        ancilla_mapping = {'Z' + str(i): q for i, q in enumerate(code.zcheck_qubits)}
        ancilla_mapping.update({'X' + str(i): q for i, q in enumerate(code.xcheck_qubits)})
        ancilla_type = {name: 'X' if name[0] == 'X' else 'Z' for name in ancilla_mapping.keys()}
        data_mapping = {q: q for q in set(code.all_qubits) - set(ancilla_mapping.values())}
        ...
        for i in range(len(cx_list)):
            q1, q2 = cx_list[i]
            cx_list[i] = (all_qubits_ids_to_names[q1], all_qubits_ids_to_names[q2])
