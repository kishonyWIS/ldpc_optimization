...
def get_BB_code():
    ...
    lz = np.load(
        f'lz_ell_{ell}_m_{m}_a1_{a1}_a2_{a2}_a3_{a3}_b1_{b1}_b2_{b2}_b3_{b3}.npy')
    return hx, hz, lz
...
def get_logical_error_rate(n, x_stabilizers, z_stabilizers, lz, p, num_shots):
    circuit, circuit_without_flag_observables = memory_experiment_circuit(
        n, x_stabilizers, z_stabilizers, lz, p, cycles_before_noise=1, cycles_with_noise=1, cycles_after_noise=1)
    ...
    decoder = BPOSD(
        circuit_without_flag_observables.detector_error_model(), max_bp_iters=20)
...
def simulated_annealing(x_stabilizers, z_stabilizers, n, lz, p, num_shots, initial_temp=1.0, cooling_rate=0.95, min_temp=0.00, max_iters=100, use_flag=False, filename=None):
    current_x, current_z = [list(row) for row in x_stabilizers], [
        list(row) for row in z_stabilizers]
    current_energy, worst_flag = get_logical_error_rate(
        n, current_x, current_z, lz, p, num_shots)
    ...
    for iteration in tqdm(range(max_iters)):
        if not use_flag:
            ...
            current, candidate = (current_x, [row[:] for row in current_x]) if random.random(
            ) < 0.5 else (current_z, [row[:] for row in current_z])
...
>>>>>>> Stashed changes
    ...
    best_x, best_z, best_logical_error_rate = simulated_annealing(x_stabilizers_best, z_stabilizers, n, lz, p,
                                                                  num_shots, max_iters=10, use_flag=True,
                                                                  initial_temp=1., cooling_rate=0.98,
                                                                  filename=f'{x_stabilizers_best}_{n}_{p}_{num_shots}')
